...
const StationCard = memo(({ station, onSelect, onToggleFavorite, isFavorite }) => {
  ...
  return (
    <div className="bg-gray-900 rounded-2xl shadow-md overflow-hidden hover:shadow-amber-900/30 hover:shadow-lg transition group cursor-pointer border border-gray-800" onClick={() => onSelect?.(station)}>
      <div className="relative bg-amber-950/30 h-36 flex items-center justify-center">
        ...
        <button className="absolute top-3 right-3 p-2 rounded-full bg-black/40 hover:bg-black z-10" onClick={(e) => { e.stopPropagation(); onToggleFavorite(station.id); }}>
      ...
      <div className="p-5">
        ...
        <button onClick={(e) => { e.stopPropagation(); onSelect?.(station); }} className="w-full bg-gray-800 hover:bg-amber-700 hover:text-white text-gray-300 font-medium py-2 rounded-lg flex items-center justify-center gap-2 transition">
          <FaEye /> View Details
        </button>
      </div>
    </div>
